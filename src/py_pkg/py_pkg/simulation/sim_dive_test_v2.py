...
class BladderMonitor(Node):
    def __init__(self):
        ...
        self.bladder_pub = self.create_publisher(Float64, self.bladder_topic, 10)
    ...
    def set_bladder(self, bladder_value):
        ...
        msg = Float64()
        msg.data = bladder_value
        self.bladder_pub.publish(msg)
        self.get_logger().info(f"Bladder set to {bladder_value}.")
...
def main(args=None):
    ...
    rosbag_process_imu = subprocess.Popen([
        "ros2", "bag", "record",
        "-o", "bag_files/dive_test_imu.bag",   # output filename
        "/model/glider_nautilus/imu"
    ])
    rosbag_process_pose = subprocess.Popen([
        "ros2", "bag", "record",
        "-o", "bag_files/dive_test_pose.bag",   # output filename
        "/model/glider_nautilus/pose"
    ])
    ...
    try:
        ...
        rosbag_process_imu.terminate()  # Sends SIGTERM to rosbag
        rosbag_process_pose.terminate()  # Sends SIGTERM to rosbag
        rosbag_process_imu.wait()       # Waits for rosbag to finish cleanup
        rosbag_process_pose.wait()       # Waits for rosbag to finish cleanup
