...
Sits behind ``bcu_node``'s stop path. Two jobs:
  ...
  - **Reassert burst.** On a mission stop the controller emits one safe-stop
    ...
    the STM latches the zero even if a single message is dropped -- the STM
    firmware has no staleness watchdog of its own, it re-ships whatever it last
    heard forever. ``stm_com_node``'s ``command_timeout_s`` dead-man is the
    unconditional backstop for that (it also covers a crashed controller, which
    a burst cannot); the burst is what makes the stop land *promptly*, within a
    tick or two rather than after the multi-second dead-man window.
...
Pure and clock-injected like ``TankLimitGuard``: the node
