"""Tier 1 unit tests for the depth_node tank-limit output clamp.

`clamp_to_tank_limits(pump_rpm, motor_open, free_open, tank_pa,
tank_empty_pa, tank_full_pa)` is the last pure decision before the BCU
wire: zero the pump and shut both valves when the commanded oil flow is
headed at a registered tank endpoint that's already been reached.
...
from py_pkg.pid.depth_node import clamp_to_tank_limits
...
class TestTowardEmpty:
    """Positive RPM drains the tank; clamp at/below the empty endpoint."""
    ...
    def test_just_above_empty_limit_passes(self):
        assert clamp_to_tank_limits(1500, 1, 0, EMPTY + 1, EMPTY, FULL) == (1500, 1, 0)


class TestTowardFull:
    """Negative RPM fills the tank; clamp at/above the full endpoint."""
    ...
    def test_just_below_full_limit_passes(self):
        assert clamp_to_tank_limits(-1500, 1, 0, FULL - 1, EMPTY, FULL) == (
            -1500,
            1,
            0,
        )
