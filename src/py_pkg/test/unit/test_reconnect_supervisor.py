...
No ROS, no paho, no clock -- (connected, rx_age, now) are handed in directly, so
each rebuild rule is asserted against synthetic observations. This is the
decision behind the MQTT bridge's self-heal: when paho's own reconnect wedges
after a yanked tether, the supervisor is what says "rebuild the client" instead
of leaving the bridge alive but mute until a reboot.
"""
...
class TestHealthyLink:
    def test_connected_with_fresh_rx_never_rebuilds(self):
        ...
        for t in range(0, 100, 5):
            assert (
                sup.should_rebuild(connected=True, rx_age_s=1.0, now=float(t)) is False
            )
    ...
    def test_fresh_boot_connected_without_rx_does_not_rebuild(self):
        ...
        # the silence window.
        sup = _sup()
        assert sup.should_rebuild(connected=True, rx_age_s=None, now=0.0) is False
        assert (
            sup.should_rebuild(connected=True, rx_age_s=None, now=10 * RX_SILENCE)
            is False
        )
...
class TestDisconnectGrace:
    def test_brief_disconnect_within_grace_holds_off(self):
        ...
        assert sup.should_rebuild(connected=False, rx_age_s=5.0, now=100.0) is False
        assert (
            sup.should_rebuild(connected=False, rx_age_s=5.0, now=100.0 + GRACE - 0.1)
            is False
        )
    ...
    def test_disconnect_past_grace_rebuilds(self):
        ...
        assert sup.should_rebuild(connected=False, rx_age_s=5.0, now=100.0) is False
        assert (
            sup.should_rebuild(connected=False, rx_age_s=5.0, now=100.0 + GRACE) is True
        )
    ...
    def test_reconnect_before_grace_clears_the_down_clock(self):
        ...
        assert sup.should_rebuild(connected=False, rx_age_s=1.0, now=100.0) is False
        assert sup.should_rebuild(connected=True, rx_age_s=1.0, now=105.0) is False
        assert sup.should_rebuild(connected=False, rx_age_s=1.0, now=109.0) is False
        # Only GRACE after the *second* drop does it escalate.
        assert (
            sup.should_rebuild(connected=False, rx_age_s=1.0, now=109.0 + GRACE) is True
        )
...
class TestStaleConnectedWedge:
    def test_connected_but_silent_past_window_rebuilds(self):
        ...
        assert (
            sup.should_rebuild(connected=True, rx_age_s=RX_SILENCE - 1.0, now=0.0)
            is False
        )
        assert (
            sup.should_rebuild(connected=True, rx_age_s=RX_SILENCE, now=0.0) is True
        )
...
class TestNoteRebuilt:
    def test_note_rebuilt_resets_the_down_clock(self):
        ...
        assert sup.should_rebuild(connected=False, rx_age_s=1.0, now=0.0) is False
        assert sup.should_rebuild(connected=False, rx_age_s=1.0, now=GRACE) is True
        sup.note_rebuilt()
        ...
        assert sup.should_rebuild(connected=False, rx_age_s=None, now=GRACE) is False
        assert (
            sup.should_rebuild(connected=False, rx_age_s=None, now=2 * GRACE - 0.1)
            is False
        )
        assert (
            sup.should_rebuild(connected=False, rx_age_s=None, now=2 * GRACE) is True
        )
